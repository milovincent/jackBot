...
if __name__ == '__main__':
    basebot.run_minibot(botname='jackBot', nickname='jackBot', log_users=True,
                        regexes={
                            '(?i)([\s\S]*?)how([\s\S]*?)win([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room)': "I'm not gonna tell you how to cheat! Mostly because I don't know :/",
                            ...
                            '(?i)([\s\S]*?)how([\s\S]*?)many([\s\S]*?)(people|players|participants)([\s\S]*?)(?!(need|have|must))([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room)': 'Most games require at least 3 players (though Guesspionage and Fibbage only require 2 (and several only require 1! ^^)) and can hold up to 8 players, though Bracketeering can hold up to 16.',
                            '(?i)([\s\S]*?)how([\s\S]*?)many([\s\S]*?)(people|players|participants)([\s\S]*?)(need|have|must)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room)': 'Most games require at least 3 players (though Guesspionage and Fibbage only require 2 (and several only require 1! ^^)).',
                            '(?i)([\s\S]*?)(what|\\bwat\\b|how(?!(many|join|play|win))|why|tell([\s\S]*?)about)([\s\S]*?)(?!(quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp))([\s\S]*?)jackbox([\s\S]*?)(?!(quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp))': 'Jackbox games are a series of games that someone hosts (ask @DoctorNumberFour or =3) and everybody can play, by opening a stream (probably at grv.to/jackboxkcd (Doctor\'s stream) or maybe at mixer.com/drownthewitch (=3\'s stream)) on a browser tab or somewhere else, where there is a 4-character code for the current game. Then they go to jackbox.tv in another browser tab or on their smartphone, and enter the code and a nickname there. Most games need 3 players or more.\nThere are euphoria themed games, as well as fast or slow-paced games, and trivia or drawing games as well. It\'s all lots of fun!',
                            '(?i)([\s\S]*?)where([\s\S]*?)(watch|stream)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room)': 'There are three locations. Most often, @DoctorNumberFour streams at grv.to/jackboxkcd, or his youtube channel, https://www.youtube.com/c/MiloSzecket_says_the_fourth_doctor_is_the_best_one/live (yes he knows how stupid the url is), and sometimes =3 streams at mixer.com/drownthewitch.',
                            ...
                            '(?i)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room).{0,2}a.{0,2}game': 'Yes!',
