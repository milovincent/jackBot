...
def linker(match, meta):
    if meta['sender'] != 'RedditLinker':
        users = re.findall(r'(?<![a-zA-Z0-9])/?u/([a-zA-Z0-9_-]{3,20})\b', meta['msg'].content, flags=0)
        for i in users:
            meta['self'].set_nickname('RedditLinker')
            meta['reply']('reddit.com/u/%s' % i)
            meta['self'].set_nickname('jackBot')
        reddits = re.findall(r'(?<![a-zA-Z0-9])/?r/([a-zA-Z0-9_-]{2,21})\b', meta['msg'].content, flags=0)
        for i in reddits:
            meta['self'].set_nickname('RedditLinker')
...
if __name__ == '__main__':
    basebot.run_minibot(botname='jackBot', nickname='jackBot', log_users=True,
                        regexes={
                            '(?i)([\s\S]*?)how([\s\S]*?)win([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K.O.|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y\b+d\b+k\b+j|you don\'?t know jack|patently stupid|split the room)': "I'm not gonna tell you how to cheat! Mostly because I don't know :/",
                            ...
                            '(?i)([\s\S]*?)how([\s\S]*?)many([\s\S]*?)(people|players|participants)([\s\S]*?)(?!(need|have|must))([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K.O.|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y\b+d\b+k\b+j|you don\'?t know jack|patently stupid|split the room)': 'Most games require at least 3 players (though Guesspionage and Fibbage only require 2 (and several only require 1! ^^)) and can hold up to 8 players, though Bracketeering can hold up to 16.',
                            ...
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(tee\b+k\b+o|t\b+?k\b+?o)': 'In Tee K. O., you and your friends draw designs, write slogans, and mix-n-match them to create t-shirts, which are then pitted against one another in a sort of battle royale! It\'s tons of fun, and you can even buy the shirts afterwards!\n3-8 players',
                            '^!players (tee\b+k\b+o|t\b+?k\b+?o)':'Tee K.O. is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(Trivia\b+Murder\b+Party|TMP)': 'This is a general trivia game, but with a unique serial killer twist! From finger removal to the Loser Wheel, much deadly fun is to be had playing Trivia Murder Party! \n1-8 players',
                            '^!players (Trivia\b+Murder\b+Party|TMP)':'Trivia Murder Party is played with 1-8 people.',
                            ...
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)civic\b+doodle': 'In Civic Doodle, the mayor of Doodle city has commissioned you (all of you) to make art to put all around the city! Take turns adding to a communal drawing, then voting on which addition is best. Maybe you\'ll even get to paint a portrait to hang in Town Hall!\n3-8 players',
                            '^!players civic\b+doodle':'Civic Doodle is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)bomb\b+corp': 'In Bomb Corp, you try to beat the clock in daily office tasks such as filing and defusing bombs. NOTE: Hard to play with lag.\n1-4 players',
                            '^!players bomb\b+corp':'Bomb Corp is played with 1-4 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)zeeple\b+dome':'In Zeeple Dome, you play a sadistic alien flinging humans around what is basically jackbox\'s version of Running Man. A gauntlet of co-op, fast-flinging fun! Unless you have lag.\n1-6 players',
                            '^!players zeeple\b+dome':'Zeeple Dome is played with 1-6 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)mad\b+verse\b+city':'In Mad Verse City, you play a robot taking part in the sickest rap battle of the century! Answer prompts for the last word of your first line, then shit yourself as you try in vain to come up with something clever to say in the second one!\n3-8 players',
                            '^!players mad\b+verse\b+city':'Mad Verse City is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)split\b+the\b+room':'In Split the Room, you are given (and expected to create) hypothetical what-if scenarios for the other players to vote on. Hosted by Schrödinger\'s cat itself!\n3-8 players',
                            '^!players split\b+the\b+room':'Split the Room is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)patently\b+stupid':'In Patently Stupid, you are given a problem to solve, and must do so with nothing but a sub-par cocktail napkin, a pen, and some ingenuity! Players then vote on designs in order to fund them!\n3-8 players',
                            '^!players patently\b+stupid':'Patently Stupid is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(y\b+d\b+k\b+j|you don\'?t know jack)':'You Don\'t Know Jack is a trivia game with a twist! It\'s like a normal trivia game, but presided over by a large streaming corporation with little-to-no backstory known as Binjpipe. Just try and figure them out!\n1-8 players',
                            '^!players (y\b+d\b+k\b+j|you don\'?t know jack)':'You Don\'t Know Jack is played with 1-8 people.',
                            '(?i)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K.O.|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y\b+d\b+k\b+j|you don\'?t know jack|patently stupid|split the room)\b+a\b+game': 'Yes!',
                            '(?i)(what|wat|how(?!(many))|why)([\s\S]*?)survive\b+the\b+internet': 'In Survive the Internet, you do just that by using your evil brain to twist someone\'s innocent comment about how selfie sticks are an abomination into something a genocidal maniac would say! (all in good fun of course:smile:)\n3-8 players',
                            '(?i)(^!help\s*@?jackBot$|(what|who|why)([\s\S]*?)jack\b+bot)': 'I tell you about Jackbox games! Just ask (using jackbox in your question)! Use:\n !jackboxspecifics for a list of games,\n !games <user> to see how many jackbox games a user has,\n !players to learn about player requirements,\n and more! I\'ve added some easter eggs as well!\nMade by @DoctorNumberFour.',
                            '(?i)([\s\S]*?)where\b+am\b+i': room,
                            '(?i)this\b+town\b+(ain\'t|aint|isn\'t|isnt|is not)\b+big\b+enough\b+for\b+(the\b+two\b+of\b+us|the both of us|us two|both of us)': tumble,
                            '(?i)gigawatt': greatScott,
                            '(?i)back\b+to\b+the\b+future': greatScott,
                            '(?i)great\b+scott': myThing, r'(?<![a-zA-Z0-9]{1}/)u/([a-zA-Z0-9_-]{3,20})\b': linker,r'(?<![a-zA-Z0-9]{1}/)r/([a-zA-Z0-9_-]{2,21})\b': linker,
                            '^!kill @jackbot$': kill,
                            '^!killall @jackbot$': killall,
                            '(?i)([\s\S]*?)where\b+is(\b+bot\b+bot|([\s\S]*?)other\b+bots)': 'BotBot is down right now, sorry! Ask =3 about it. It probably won\'t be up for some time, so you can run your own using one of the many bot libraries other people have made, such as yaboli (from @Garmy) and basebot (what jackBot uses, made by @Xyzzy). you do have to take care of server-side stuff though. :/',
                            ...
                            '(?i)who([\s\S]*?)jack\b+bot': 'I was made by @DoctorNumberFour!',
                            '(?i)(when|where)([\s\S]*?)jack\b+bot': 'I am an eldritch abomination from a land outside of time. Do not ask me such trivial things.',
                            '(?i)([\s\S]*?)is\b+this\b+real\b+life':'Yes',
                            'how([\s\S]*?)base\b+bot':'Ask @Xyzzy.',
