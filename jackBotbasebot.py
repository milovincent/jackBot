...
if __name__ == '__main__':
    basebot.run_minibot(botname='jackBot', nickname='jackBot', log_users=True,
                        regexes={
                            '(?i)([\s\S]*?)how([\s\S]*?)win([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|\bt(?:ee)?\.?\s?k\.?\s?o\b|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y\.?\s?d\.?\s?k\.?\s?j|you don\'?t know jack|patently stupid|split the room)': "I'm not gonna tell you how to cheat! Mostly because I don't know :/",
                            ...
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(\bt(?:ee)?\.?\s?k\.?\s?o\b)': 'In Tee K. O., you and your friends draw designs, write slogans, and mix-n-match them to create t-shirts, which are then pitted against one another in a sort of battle royale! It\'s tons of fun, and you can even buy the shirts afterwards!\n3-8 players',
                            '^!players (\bt(?:ee)?\.?\s?k\.?\s?o\b)':'Tee K.O. is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(Trivia\.?\s?Murder\.?\s?Party|TMP)': 'This is a general trivia game, but with a unique serial killer twist! From finger removal to the Loser Wheel, much deadly fun is to be had playing Trivia Murder Party! \n1-8 players',
                            ...
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)civic\.?\s?doodle': 'In Civic Doodle, the mayor of Doodle city has commissioned you (all of you) to make art to put all around the city! Take turns adding to a communal drawing, then voting on which addition is best. Maybe you\'ll even get to paint a portrait to hang in Town Hall!\n3-8 players',
                            '^!players civic\.?\s?doodle':'Civic Doodle is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)bomb\.?\s?corp': 'In Bomb Corp, you try to beat the clock in daily office tasks such as filing and defusing bombs. NOTE: Hard to play with lag.\n1-4 players',
                            '^!players bomb\.?\s?corp':'Bomb Corp is played with 1-4 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)zeeple\.?\s?dome':'In Zeeple Dome, you play a sadistic alien flinging humans around what is basically jackbox\'s version of Running Man. A gauntlet of co-op, fast-flinging fun! Unless you have lag.\n1-6 players',
                            '^!players zeeple\.?\s?dome':'Zeeple Dome is played with 1-6 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)mad\.?\s?verse\.?\s?city':'In Mad Verse City, you play a robot taking part in the sickest rap battle of the century! Answer prompts for the last word of your first line, then shit yourself as you try in vain to come up with something clever to say in the second one!\n3-8 players',
                            '^!players mad\.?\s?verse\.?\s?city':'Mad Verse City is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)split\.?\s?the\.?\s?room':'In Split the Room, you are given (and expected to create) hypothetical what-if scenarios for the other players to vote on. Hosted by Schrödinger\'s cat itself!\n3-8 players',
                            '^!players split\.?\s?the\.?\s?room':'Split the Room is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)patently\.?\s?stupid':'In Patently Stupid, you are given a problem to solve, and must do so with nothing but a sub-par cocktail napkin, a pen, and some ingenuity! Players then vote on designs in order to fund them!\n3-8 players',
                            '^!players patently\.?\s?stupid':'Patently Stupid is played with 3-8 people.',
                            '(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(y\.?\s?d\.?\s?k\.?\s?j|you don\'?t know jack)':'You Don\'t Know Jack is a trivia game with a twist! It\'s like a normal trivia game, but presided over by a large streaming corporation with little-to-no backstory known as Binjpipe. Just try and figure them out!\n1-8 players',
                            '^!players (y\.?\s?d\.?\s?k\.?\s?j|you don\'?t know jack)':'You Don\'t Know Jack is played with 1-8 people.',
                            '(?i)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|\bt(?:ee)?\.?\s?k\.?\s?o\b|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y\.?\s?d\.?\s?k\.?\s?j|you don\'?t know jack|patently stupid|split the room)\.?\s?a\.?\s?game': 'Yes!',
                            '(?i)(what|wat|how(?!(many))|why)([\s\S]*?)survive\.?\s?the\.?\s?internet': 'In Survive the Internet, you do just that by using your evil brain to twist someone\'s innocent comment about how selfie sticks are an abomination into something a genocidal maniac would say! (all in good fun of course:smile:)\n3-8 players',
                            '(?i)(^!help\s*@?jackBot$|(what|who|why)([\s\S]*?)jack\.?\s?bot)': 'I tell you about Jackbox games! Just ask (using jackbox in your question)! Use:\n !jackboxspecifics for a list of games,\n !games <user> to see how many jackbox games a user has,\n !players to learn about player requirements,\n and more! I\'ve added some easter eggs as well!\nMade by @DoctorNumberFour using Xyzzy\'s basebot library: https://github.com/CylonicRaider/basebot.',
                            '(?i)([\s\S]*?)where\.?\s?am\.?\s?i': room,
                            '(?i)this\.?\s?town\.?\s?(ain\'t|aint|isn\'t|isnt|is not)\.?\s?big\.?\s?enough\.?\s?for\.?\s?(the\.?\s?two\.?\s?of\.?\s?us|the both of us|us two|both of us)': tumble,
                            '(?i)gigawatt': greatScott,
                            '(?i)back\.?\s?to\.?\s?the\.?\s?future': greatScott,
                            '(?i)great\.?\s?scott': myThing, r'(?<![a-zA-Z0-9]{1}/)u/([a-zA-Z0-9_-]{3,20})\b': linker,r'(?<![a-zA-Z0-9]{1}/)r/([a-zA-Z0-9_-]{2,21})\b': linker,
                            '^!kill @jackbot$': kill,
                            '^!killall @jackbot$': killall,
                            '(?i)([\s\S]*?)where\.?\s?is(\.?\s?bot\.?\s?bot|([\s\S]*?)other\.?\s?bots)': 'BotBot is down right now, sorry! Ask =3 about it. It probably won\'t be up for some time, so you can run your own using one of the many bot libraries other people have made, such as yaboli (from @Garmy) and basebot (what jackBot uses, made by @Xyzzy). you do have to take care of server-side stuff though. :/',
