...
def linker(match, meta):
    if meta['sender'] != 'RedditLinker':
        users = re.findall('(?<![a-zA-Z0-9]{1}/)u/([a-zA-Z0-9_-]{3,20})', meta['msg'].content, flags=0)
        for i in users:
            meta['self'].set_nickname('RedditLinker')
            meta['reply']('reddit.com/u/%s' % i)
            meta['self'].set_nickname('jackBot')
        reddits = re.findall('(?<![a-zA-Z0-9]{1}/)r/([a-zA-Z0-9_-]{2,21})', meta['msg'].content, flags=0)
        for i in reddits:
            meta['self'].set_nickname('RedditLinker')
...
if __name__ == '__main__':
    basebot.run_minibot(botname='jackBot', nickname='jackBot', log_users=True,
                        regexes={
                            ...
                            '(?i)great.{0,2}scott': myThing, '(?<![a-zA-Z0-9]{1}/)u/([a-zA-Z0-9_-]{3,20})': linker,'(?<![a-zA-Z0-9]{1}/)r/([a-zA-Z0-9_-]{2,21})': linker,
                            '^!kill @jackbot$': kill,
