...
if __name__ == '__main__':
    basebot.run_minibot(botname='jackBot', nickname='jackBot', log_users=True,
                        regexes={
                            r'(?i)([\s\S]*?)how([\s\S]*?)win([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tee K\.O\.|\btee\.?\sk\.?\so\b|\bt\.?\s?k\.?\s?o\b|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room)': "I'm not gonna tell you how to cheat! Mostly because I don't know :/",
                            ...
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(tee.?\sk.?\so|t.?\s?k.?\s?o)': 'In tee K\.O\.|\btee\.?\sk\.?\so\b|\bt\.?\s?k\.?\s?o\b|tko., you and your friends draw designs, write slogans, and mix-n-match them to create t-shirts, which are then pitted against one another in a sort of battle royale! It\'s tons of fun, and you can even buy the shirts afterwards!\n3-8 players',
                            '^!players (tee.?\sk.?\so|t.?\s?k.?\s?o)':'tee K\.O\.|\btee\.?\sk\.?\so\b|\bt\.?\s?k\.?\s?o\b|tko is played with 3-8 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(Trivia.{0,2}Murder.{0,2}Party|TMP)': 'This is a general trivia game, but with a unique serial killer twist! From finger removal to the Loser Wheel, much deadly fun is to be had playing Trivia Murder Party! \n1-8 players',
                            ...
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)civic.{0,2}doodle': 'In Civic Doodle, the mayor of Doodle city has commissioned you (all of you) to make art to put all around the city! Take turns adding to a communal drawing, then voting on which addition is best. Maybe you\'ll even get to paint a portrait to hang in Town Hall!\n3-8 players',
                            '^!players civic.{0,2}doodle':'Civic Doodle is played with 3-8 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)bomb.{0,2}corp': 'In Bomb Corp, you try to beat the clock in daily office tasks such as filing and defusing bombs. NOTE: Hard to play with lag.\n1-4 players',
                            '^!players bomb.{0,2}corp':'Bomb Corp is played with 1-4 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)zeeple.{0,2}dome':'In Zeeple Dome, you play a sadistic alien flinging humans around what is basically jackbox\'s version of Running Man. A gauntlet of co-op, fast-flinging fun! Unless you have lag.\n1-6 players',
                            '^!players zeeple.{0,2}dome':'Zeeple Dome is played with 1-6 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)mad.{0,2}verse.{0,2}city':'In Mad Verse City, you play a robot taking part in the sickest rap battle of the century! Answer prompts for the last word of your first line, then shit yourself as you try in vain to come up with something clever to say in the second one!\n3-8 players',
                            '^!players mad.{0,2}verse.{0,2}city':'Mad Verse City is played with 3-8 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)split.{0,2}the.{0,2}room':'In Split the Room, you are given (and expected to create) hypothetical what-if scenarios for the other players to vote on. Hosted by Schrödinger\'s cat itself!\n3-8 players',
                            '^!players split.{0,2}the.{0,2}room':'Split the Room is played with 3-8 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)patently.{0,2}stupid':'In Patently Stupid, you are given a problem to solve, and must do so with nothing but a sub-par cocktail napkin, a pen, and some ingenuity! Players then vote on designs in order to fund them!\n3-8 players',
                            '^!players patently.{0,2}stupid':'Patently Stupid is played with 3-8 people.',
                            r'(?i)([\s\S]*?)(what|wat|how(?!(many))|why)([\s\S]*?)(y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack)':'You Don\'t Know Jack is a trivia game with a twist! It\'s like a normal trivia game, but presided over by a large streaming corporation with little-to-no backstory known as Binjpipe. Just try and figure them out!\n1-8 players',
                            '^!players (y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack)':'You Don\'t Know Jack is played with 1-8 people.',
                            r'(?i)([\s\S]*?)(jackbox|quiplash|bracketeering|earwax|trivia murder party|t.m.p.|tmp|tee K\.O\.|tee\.?\sk\.?\so|t\.?\s?k\.?\s?o|tee K\.O\.|\btee\.?\sk\.?\so\b|\bt\.?\s?k\.?\s?o\b|tko|civic doodle|fibbage|guesspionage|bidiots|survive the internet|bomb corp|zeeple dome|mad verse city|y.{0,2}d.{0,2}k.{0,2}j|you don\'?t know jack|patently stupid|split the room).{0,2}a.{0,2}game': 'Yes!',
                            r'(?i)(what|wat|how(?!(many))|why)([\s\S]*?)survive.{0,2}the.{0,2}internet': 'In Survive the Internet, you do just that by using your evil brain to twist someone\'s innocent comment about how selfie sticks are an abomination into something a genocidal maniac would say! (all in good fun of course:smile:)\n3-8 players',
                            r'(?i)(^!help\s*@?jackBot$|(what|who|why)([\s\S]*?)jack.{0,2}bot)': 'I tell you about Jackbox games! Just ask (using jackbox in your question)! Use:\n !jackboxspecifics for a list of games,\n !games <user> to see how many jackbox games a user has,\n !players to learn about player requirements,\n and more! I\'ve added some easter eggs as well!\nMade by @DoctorNumberFour.',
                            r'(?i)([\s\S]*?)where.{0,2}am.{0,2}i': room,
                            r'(?i)this.{0,2}town.{0,2}(ain\'t|aint|isn\'t|isnt|is not).{0,2}big.{0,2}enough.{0,2}for.{0,2}(the.{0,2}two.{0,2}of.{0,2}us|the both of us|us two|both of us)': tumble,
                            r'(?i)gigawatt': greatScott,
                            r'(?i)back.{0,2}to.{0,2}the.{0,2}future': greatScott,
                            r'(?i)great.{0,2}scott': myThing, r'(?<![a-zA-Z0-9]{1}/)u/([a-zA-Z0-9_-]{3,20})\b': linker,r'(?<![a-zA-Z0-9]{1}/)r/([a-zA-Z0-9_-]{2,21})\b': linker,
                            '^!kill @jackbot$': kill,
                            '^!killall @jackbot$': killall,
                            r'(?i)([\s\S]*?)where.{0,2}is(.{0,2}bot.{0,2}bot|([\s\S]*?)other.{0,2}bots)': 'BotBot is down right now, sorry! Ask =3 about it. It probably won\'t be up for some time, so you can run your own using one of the many bot libraries other people have made, such as yaboli (from @Garmy) and basebot (what jackBot uses, made by @Xyzzy). you do have to take care of server-side stuff though. :/',
                            ...
                            r'(?i)who([\s\S]*?)jack.{0,2}bot': 'I was made by @DoctorNumberFour!',
                            r'(?i)(when|where)([\s\S]*?)jack.{0,2}bot': 'I am an eldritch abomination from a land outside of time. Do not ask me such trivial things.',
                            r'(?i)([\s\S]*?)is.{0,2}this.{0,2}real.{0,2}life':'Yes',
